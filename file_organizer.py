...
class FileOrganizer:
    ...
    def __init__(self, source_dir, target_dir=None, organize_by_date=True, 
        ...
        # 기존 로그 파일이 있으면 로드 (추가 모드)
        if self.log_file.exists() and not dry_run:
            try:
                with open(self.log_file, 'r', encoding='utf-8') as f:
                    self.move_log = json.load(f)
            except (json.JSONDecodeError, IOError):
                # 로그 파일이 손상되었거나 읽을 수 없는 경우 새로 시작
                self.move_log = []
    ...
    def save_log(self):
        ...
        try:
            with open(self.log_file, 'w', encoding='utf-8') as f:
                json.dump(self.move_log, f, ensure_ascii=False, indent=2)
        except IOError as e:
            print(f"경고: 로그 파일 저장 실패: {e}", file=sys.stderr)
    ...
    def organize_file(self, file_path):
        ...
        if not self.dry_run:
            try:
                ...
                # 이동 실행
                shutil.move(original_path_str, target_path_str)
                
                # 로그에 추가 (상대 경로로 저장)
                try:
                    original_relative = str(file_path.relative_to(self.source_dir))
                    target_relative = str(target_path.relative_to(self.target_dir))
                except ValueError:
                    # 상대 경로를 만들 수 없는 경우 절대 경로 사용
                    original_relative = original_path_str
                    target_relative = target_path_str
                
                self.move_log.append({
                    'original': original_relative,
                    'moved_to': target_relative,
...
사용 예시:
  ...
  # 날짜별로만 정리
  python3 file_organizer.py ~/Downloads --no-type
  
  # 유형별로만 정리
  python3 file_organizer.py ~/Downloads --no-date
        """
    )
